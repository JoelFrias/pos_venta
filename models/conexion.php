...
function gestionarErrorConexion($mensaje) {
    ...
    error_log("[" . date("Y-m-d H:i:s") . "] Error de conexión a la BD: " . $mensaje, 3, "db_errors.log");
...
// Intentar crear la conexión con manejo de excepciones
try {
    // Crear conexión
...
// Si se requiere en otras partes de la aplicación, se puede verificar si la conexión está activa
function verificarConexion($conn) {
    if (!$conn || $conn->connect_error) {
        gestionarErrorConexion("Conexión perdida o inválida");
        return false;
    }
    return true;
}
